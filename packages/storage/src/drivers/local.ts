...
export default class LocalStorage implements Storage {
  ...
  getItem<T>(
    ...
    if (typeof key === 'string') {
      ...
      return new Promise((resolve, reject) => {
        const value = localStorage.getItem(this.getStorageKey(key.key));
        if (value) {
  ...
  has(key: string): boolean {
    const result = localStorage.getItem(this.getStorageKey(key));
    return result !== null;
  }
  ...
  removeItem(
    ...
    if (typeof key === 'string') {
      ...
      return new Promise((resolve, reject) => {
        const result = localStorage.getItem(this.getStorageKey(key.key));
        if (result) {
          localStorage.removeItem(this.getStorageKey(key.key));
          resolve();
